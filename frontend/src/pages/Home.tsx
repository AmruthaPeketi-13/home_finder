...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      setIsLoading(true);
      try {
        const { getProperties } = await import('@/lib/propertyService');
        const data = await getProperties();

        // Transform backend data to match frontend format
        const transformedProperties = data.map((prop, index) => ({
          id: prop._id,
          title: prop.title,
          location: prop.location,
          price: formatPrice(prop.price, prop.type),
          priceLabel: getPriceLabel(prop.type),
          bedrooms: 3, // Default values since backend doesn't have these yet
          bathrooms: 2,
          sqft: 1450,
